...
(function(){
  ...
  mod.directive('sfVirtualRepeat', ['$log', '$rootElement', function($log, $rootElement){
    ...
    //
    function setViewportCss(viewport, overflowValue){
      var viewportCss = {'overflow': overflowValue},
    ...
    function sfVirtualRepeatCompile(element, attr, linker) {
      ...
      function sfVirtualRepeatPostLink(scope, iterStartElement, attrs){
        ...
        var isScrollBar = attrs.sfScrollBar;
        ...
        setContentCss(dom.content);
          if (angular.isDefined(isScrollBar) && isScrollBar=="O") {
              setViewportCss(dom.viewport, 'auto');
              console.log('isScrollBar : ' + isScrollBar);
              isScrollBar="O";
          } else {
              setViewportCss(dom.viewport, 'hidden');
              console.log('isScrollBar false : ' + isScrollBar);
              isScrollBar="N";
            scope.$on('virtual-scroll', function (event, evt) {
                dom.viewport.scrollTop(evt.target.scrollTop);
                console.log("Receive event - " + evt.target.scrollTop);
            });
          }
        ...
        function sfVirtualRepeatOnScroll(evt){
          if( !rowHeight ){
            ...
            if (isScrollBar=="O") {
                console.log('isScrollBar : ' + isScrollBar);
                isScrollBar=="O";
                scope.$broadcast('virtual-scroll', evt);
                console.log("Boradcast evt");
            }

            //var otherEvt = new evt.originalEvent.constructor(evt.originalEvent.type, evt.originalEvent);
            //otherElt.dispatchEvent(otherEvt);
